# Health Monitoring

[Link to SAP Cloud ALM tenant](https://cs261-n5i2x9xc.eu10-004.alm.cloud.sap/launchpad#Shell-home)

In the Health Monitoring application, you can check the health of your monitored cloud service and technical systems from an application and customer perspective.

![Cloud ALM Operations Home](images/CALM_Home_HM.png)

## Scope Selection and Home

- Open the Scope Selector
![Health Monitoring Scope Selector](images/HM_Scope_Selector.png)

- Select Service “SAPTechEd-HandsOn-CF” and apply your selection
![Health Monitoring Scope Selected](images/HM_Scope_Selected.png)
 
Result: Based on your selection, you will see a card for services of the type “SAP BTP, Cloud Foundry environment” in the Overview area. 
![Health Monitoring Overview](images/HM_Overview.png)
 
## Explore Metrics

- Home
![Health Monitoring Overview](images/HM_Overview.png)

- Select Service Type “SAP BTP, Cloud Foundry environment”
![Health Monitoring Services/Systems](images/HM_Service_Details.png)
 
- Select “SAPTechEd-HandsOn-CF”
![Health Monitoring Metrics](images/HM_Metrics.png)
 
- Select Metric “Memory: Eden Space (Java)”
![Health Monitoring Metric Selected](images/HM_Metric_Selected.png)
 
- Open History and analyse the trend
![Health Monitoring Metric History](images/HM_Metric_History.png)

## Use In-App Help

- Starting Point Health Monitoring Home or also from other areas:
![Health Monitoring Overview](images/HM_Overview.png)
  
- Open In-App Help
![Health Monitoring In App Help](images/HM_InAppHelp.png)
  
- Result
![Health Monitoring In App Help Active](images/HM_InAppHelp_Active.png)
 
 

**Next Step**\
[Proceed to Cloud Logging](/exercises/ex1/1_HealthMonitoring_CloudLogging.md)


**Additional Links**\
[Session Overview](/README.md#overview)
