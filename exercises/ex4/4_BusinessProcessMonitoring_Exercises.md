# Business Process Monitoring Exercises

Disclaimer: These tasks contain the relevant steps - the relevant solution are embedded.

## 1.1 Create your Custom KPI (Guided Excercise)

...

ZKPITBP001.  <participant number>
ZKPITBP045

001 Travel Bookings in Status Open

Same definition as for ZKPITB0001

## 1.2 Check Data Collection (Guided Excercise)

...

>> Request the instructor to trigger the data collection

## 1.3 Create an Event to be alerted (Guided Excercise)

...

## 1.4 Adjust Graph and work with Views (Guided Excercise)

...

## 1.5 Explore Guided Tours (Guided Excercise)


**** Most likely no separate solutions for BPMon
**Next Step**
[Refer to the Solutions](/exercises/ex4/4_BusinessProcessMonitoring_Solutions.md)
**** Most likely no separate solutions for BPMon

**Additional Links**
[Business process Monitoring Overview](/exercises/ex4/4_BusinessProcessMonitoring_Base.md)
[Session Overview](/README.md#overview)

**** link to Thank you page & feedback?


*******  *     *      ***      *      *   *    *     
   *     *     *     *   *     * *    *   *  *
   *     *******    *******    *   *  *   *
   *     *     *   *       *   *    * *   *  * 
   *     *     *  *         *  *    * *   *    *         
 
 
 
    TM C SJ NK